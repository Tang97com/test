import { getmsOrdrlist,getsmsOrdrlist } from '../../../api/api';
Page({
  data: {
    list: [],
    page: 2, 
  ...
  getData({page, type, id}) {
    ...
    getmsOrdrlist({page, type, id, ordertype: this.data.ordertype}).then(res => {
      console.log(res)
      wx.hideLoading()
      if (res.code === 200 ) {
    ...
    getsmsOrdrlist({page, type, id, ordertype: this.data.ordertype}).then(res => {
      console.log(res)
      wx.hideLoading()
      if (res.code === 200 ) {
        console.log("请求的页数", page, "一共", res.data.last_page)
